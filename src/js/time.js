...
export const time = () => {
    ...
    const createTimeListItem = (title, details) => (
        `<h3>${title}</h3>
         ...
         Bắt đầu lúc ${details.hours.start} giờ</p>`
    );

    marriageDiv.innerHTML = createTimeListItem('Tiệc cưới', data.time.marriage);
    // receptionDiv.innerHTML = createTimeListItem('Lễ thành hôn', data.time.reception);
